...
/* Helper to remove a symbol from a symtab, freeing its memory. */
static void
symtab_remove_symbol(struct shash *st, const char *name)
{
    struct expr_symbol *symbol = shash_find_and_delete(st, name);
    if (symbol) {
        free(symbol->name);
        free(symbol->prereqs);
...
{
    /* Clone the normal symbol table as our starting point. */
    shash_init(acl_symtab);
    expr_symtab_clone(acl_symtab, &symtab);
    ...
    symtab_remove_symbol(acl_symtab, "tcp.src");
    symtab_remove_symbol(acl_symtab, "tcp.dst");
    symtab_remove_symbol(acl_symtab, "tcp");
    symtab_remove_symbol(acl_symtab, "udp.src");
    symtab_remove_symbol(acl_symtab, "udp.dst");
    symtab_remove_symbol(acl_symtab, "udp");
    symtab_remove_symbol(acl_symtab, "sctp.src");
    symtab_remove_symbol(acl_symtab, "sctp.dst");
    symtab_remove_symbol(acl_symtab, "sctp");
    ...
    expr_symtab_add_predicate(acl_symtab, "tcp", "ct.new && !ct.inv && ct_proto == 6");
    ...
    expr_symtab_add_predicate(acl_symtab, "udp", "ct.new && !ct.inv && ct_proto == 17");
    ...
    expr_symtab_add_predicate(acl_symtab, "sctp", "ct.new && !ct.inv && ct_proto == 132");
                      ...
                      bool *pg_addr_set_ref)
{
     ...
     * If the external_ids contains "acl-ct-translation"="true",
     * we use the alternative symbol table that maps L4 fields to CT fields. */
    const char *use_ct_translation =
        smap_get(&lflow->external_ids, "acl-ct-translation");
    bool using_ct_symtab = use_ct_translation &&
                           !strcmp(use_ct_translation, "true");
    struct shash *symtab_to_use = using_ct_symtab ? &acl_ct_symtab : &symtab;

    if (strstr(lflow->match, "udp")) {
        VLOG_DBG("lflow match='%s', acl-ct-translation='%s', using_ct_symtab=%d",
                 lflow->match,
                 use_ct_translation ? use_ct_translation : "(null)",
                 using_ct_symtab);
    }
